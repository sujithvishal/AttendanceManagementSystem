...
def extract_faces(img):
    if img is not None and img.any():
        ...
        return list(face_points)  # Convert face_points to a list
    else:
        return []
        ...
        if frame is not None:
            if extract_faces(frame) != []:
                ...
                cv2.putText(frame, f'{identified_person}', (30, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 20), 2,
                            cv2.LINE_AA)
                # Break the loop and close the window after recognizing a face
                break
        cv2.imshow('Attendance', frame)
        if cv2.waitKey(1) == 27 or time.time() - start_time > 10:  # Close the window after 10 seconds if no face is recognized
            break
    cap.release()
